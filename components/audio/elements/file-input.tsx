...
export function AudioFileInput({ label = 'Load a local audio file:', className, ...props }: AudioFileInputProps) {
  ...
  return (
    <div className="flex flex-col gap-2" data-state={isDisabled ? 'disabled' : 'enabled'}>
      ...
      <Input
        ...
        {...props}
      />
      {isLoading && <span className="text-muted-foreground text-xs">Loading audio engine...</span>}
    </div>
  )
}
